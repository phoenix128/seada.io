...
import type { FlattenedItem, TreeItem, TreeItems } from '@seada.io/sortable-tree/types';
...
export function buildTree<T extends Record<string, any>>(flattenedItems: FlattenedItem<T>[]): TreeItems<T> {
...
export function getChildCount<T extends Record<string, any>>(items: TreeItems<T>, id: UniqueIdentifier) {
    if (!id) {
        return 0;
    }
