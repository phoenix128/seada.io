...
export interface IUseSearchProductsAdapter {
    (): IAsyncActionCall<(pageData: IPageData, search: string) => Promise<IProductData[]>>;
}

const useSchemaSearchProductsPort: IUseAdapterProxyHook<IUseSearchProductsAdapter> = (sourceAdapterCode: string) => {
    const fallbackHook = useAsyncActionCallStaticResponse<IProductData[]>([]);

    return useContextUnawareAdapterHook<IUseSearchProductsAdapter>(
        sourceAdapterCode,
        catalogPortClass,
        'use-schema-search-products',
        fallbackHook
    );
};

export default useSchemaSearchProductsPort;
